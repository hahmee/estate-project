...
const MultiRangeSlider = ({
  ...
  const stepRef = useRef(step); // 초기값 설정
  ...
  const updateStep = (event) => {
    if (stepCondition) {
      const newStep = stepCondition(event);
      stepRef.current = newStep;
      setStepVal(newStep); // 보여주기용
    }
  };
    ...
    <div className="multiRangeSlider">
      <input
        ...
        onChange={(event) => {
          ...
          const value = Math.min(Number(event.target.value), maxVal - stepVal);
          setMinVal(value);
        }}
